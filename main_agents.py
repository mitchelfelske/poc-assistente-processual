...
if __name__ == "__main__":
    # Exemplo básico de uso sequencial
    ingestao = AgenteIngestao()
    analise = AgenteAnalise()
    proposicao = AgenteProposicao()
    revisao = AgenteRevisao()

    # Ingestão
    processo = ingestao.executar("data/pecas_prestacao_contas")

    # Análise
    infos = analise.executar(processo)
    ...
    ato = proposicao.executar("técnico", "despacho_instrucao", resumo)

    # Revisão
    resultado_revisao = revisao.executar(ato, infos)
